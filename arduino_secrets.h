/******************************************************************************
   LIVE OBJECTS CREDENTIALS
 ******************************************************************************/
// Paste below your API key (see Configuration > API keys on the portal).
// You API key must have at least the predefined 'MQTT Device' rights profile
// (alternatively: 'Device Access' read + write rights if need to customise the rights).
// Please note that you *must* use a TLS connection (MQTTS) if you grant more rights to the API key.
const char SECRET_LIVEOBJECTS_API_KEY[] = "...";


/******************************************************************************
   CELLULAR CREDENTIALS
 ******************************************************************************/
const char SECRET_PIN[] = "";		// unless PIN is deactivated, specify your SIM card PIN number
const char SECRET_APN[] = "";					// specify the APN name (if needed)
const char SECRET_GSM_USER[] = ""; 		// specify the username for your APN (if needed)
const char SECRET_GSM_PASS[] = "";		// specify the password for your APN (if needed)


/******************************************************************************
   LORA CREDENTIALS
 ******************************************************************************/
const char SECRET_APP_EUI[] = "";
const char SECRET_APP_KEY[] = "";


/******************************************************************************
   WIFI CREDENTIALS
 ******************************************************************************/
const char SECRET_SSID[] = "";
const char SECRET_PASS[] = "";
